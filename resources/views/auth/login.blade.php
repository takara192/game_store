...
<div class="flex justify-center grow w-full relative">
    ...
    <div class="absolute z-10 flex flex-col space-y-6 items-start top-[100px]">
        ...
        <div class=" w-[700px] bg-[#171a21] rounded-md py-8 px-8">
            @if(session('success'))
                <div class="my-4 text-sm text-green-300 font-medium ">
                    {{ session('success') }}
                </div>
            @endif
            <form class="flex flex-col  text-black" method="POST" action="{{ route('auth.store') }}">
                ...
                <div class="mt-4 flex justify-center items-center">
                    <button
                        class="py-3 px-30 bg-gradient-to-r from-[#06BFFF] to-[#2D73FF] rounded-xs  text-white font-medium cursor-pointer text-base">
                        Sign in
                    </button>
                </div>
                @if(session('error'))
                ...
                <div class="flex flex-col items-center text-white mt-5">
                    ...
                    <a href="{{ route('register') }}"
                       class="rounded-xs bg-gradient-to-r from-[#06BFFF] to-[#2D73FF] px-4 py-1 mt-3 cursor-pointer">
                        Create an account
                    </a>
                </div>
            </form>
